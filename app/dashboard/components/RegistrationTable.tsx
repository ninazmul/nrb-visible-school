...
import { Trash, Eye, Edit2, BadgeCheck } from "lucide-react";
...
export const RegistrationTable = ({
  ...
  const handleConfirmPayment = async (registration: RegistrationItem) => {
    try {
      setLoading(`payment-${registration._id}`, true);

      const confirmed = await confirmRegistrationPayment(registration._id, {
        transactionId:
          registration.transactionId?.trim() || `MANUAL-${Date.now()}`,
        paymentMethod: registration.paymentMethod?.trim() || "Other",
      });
      ...
      setList((prev) =>
        prev.map((r) =>
          r._id === confirmed._id ? normalizeRegistration(confirmed) : r,
        ),
      );
      ...
      setLoading(`payment-${registration._id}`, false);
    }
  };
  ...
  return (
    <div className="space-y-4">
              ...
              <TableCell className="flex justify-end gap-2">
                ...
                {r.paymentStatus !== "Paid" && (
                  <Button
                    variant="ghost"
                    size="icon"
                    disabled={loadingMap[`payment-${r._id}`]}
      ...
      {viewModalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            {viewModalData.idProof && (
                <Image
                  src={viewModalData.idProof}
                  height={500}
                  width={500}
      ...
      {paymentConfirmId && (
        <Dialog open onOpenChange={() => setPaymentConfirmId(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Confirm Payment</DialogTitle>
            </DialogHeader>

            <p>Are you sure you want to confirm this registration's payment?</p>
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => {
                  const reg = list.find((r) => r._id === paymentConfirmId);
                  if (reg) handleConfirmPayment(reg);
